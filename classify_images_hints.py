...
def classify_images(images_dir, results_dic, model):
    ...
    for key in results_dic:
       ...
       # set labels to lowercase (lower) and stripping off whitespace(strip)
        ...
        word_label= model_label.split("_")
...
for idx in range (0, len(key), 1):
    ...
    # classifier labels
    if key[idx] not in results_dic:
        results_dic[key[idx]] = [ truth[idx], model_label[idx] ]
    ...
    if truth[idx] in model_label[idx]:
        results_dic[key[idx]].append(1)

    # if pet image label was NOT found then there is no match
    else:
        results_dic[key[idx]].append(0)
...
for idx in range (0, len(key), 1):
    ...
    results_dic[key[idx]].extend([pet_label_is_dog[idx], 
